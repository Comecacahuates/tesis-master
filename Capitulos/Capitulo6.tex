...
\DontPrintSemicolon
\LinesNumbered
    \KwData{$\{a,b\}$: Arista inicial, $\mathbf{G}$: Grafo vial, $NV$:
    Conjunto de vértices no disponibles, $NE$: Conjunto de aristas inactivas}
    \Begin{
        $n \gets|\mathbf{G}.\mathbf{V}|$
        \tcp*{Número de vértices en el grafo}
        \ForEach{$u \in \mathbf{G}.\mathbf{V}$} {
            $distancias[u] \gets \infty$ \;
        }
        $distancias[a] \gets 0$ \;
        $distancias[b] \gets 0$ \;
        $S \gets \mathbf{G.V} \setminus \{a,b\}$
        \tcp*{Vértices que faltan por procesar}
        \ForEach(\tcp*[f]{Para cada vértice adyacente a $a$}){$u \in
        \mathbf{G}.\mathbf{V} \mid \{a,u\} \in \mathbf{G}.\mathbf{E}$}{
            \If{$u \neq b$ y $\{a,u\} \notin NE$}{
                \If{$u \notin NV$}{
                    $distancias[u] \gets calcularPeso(\{a,b\}, \{a,u\})$ \;
                }
                $predecesores[u] \gets a$ \;
            }
        }
        \ForEach(\tcp*[f]{Para cada vértice adyacente a $b$}){$u \in
        \mathbf{G}.\mathbf{V} \mid \{b,u\} \in \mathbf{G}.\mathbf{V}$}{
            \If{$u \neq a$ y $\{b,u\} \notin NE$}{
                $peso \gets calcularPeso(\{a,b\}, \{b,u\}$) \;
                \If{$peso < distancias[u]$}{
                    \If{$u \notin NV$}{
                        $distancias[u] \gets peso$ \;
                    }
                    $predecesores[u] \gets b$ \;
                }
            }
        }
        \While{$S \neq \emptyset$}{
            \eIf{$\exists u \in S \mid distancias[u] \neq
            \infty$}{
                $u \gets$ vértice restante con menor distancia \;
                \ForEach(\tcp*[f]{Para cada vértice adyacente a $v$}){$v \in
                \mathbf{G}.\mathbf{V} \mid \{u,v\} \in \mathbf{G}.\mathbf{E}$}{
                    \If{$v \neq predecesores[u]$ y $v \notin NV$ y $\{u,v\}
                    \notin NE$}{
                        $peso \gets calcularPeso(\{predecesores[u],u\},
                        \{u,v\})$ \;
                        \If{$distancias[v] > distancias[u] + peso$}{
                            $distancias[v] \gets distancias[u] + peso$ \;
                            $predecesores[v] \gets u$ \;
                        }
                    }
                }
                $S \gets S \setminus \{u\}$ \;
...
En el algoritmo \ref{alg:ruta_mas_corta} es el algoritmo propuesto, donde se
